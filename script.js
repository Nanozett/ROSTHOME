...
async function addDeviceToServer(deviceData) {
    try {
        ...
        const newDevice = {
            ...
            battery: deviceData.type === 'Робот-пылесос' ? 100 : null,
    ...
    document.querySelectorAll('.device-card').forEach(card => {
        ...
        card.addEventListener('click', (e) => {
            if (e.target.classList.contains('toggle-btn') || e.target.classList.contains('charge-btn') ||
                ...
                e.target.classList.contains('temp-slider') || e.target.classList.contains('delete-btn')) return;
            selectDevice(devId);
        });
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', async (e) => {
                ...
                await deleteDeviceFromServer(id);
            });
        }
...
// --- Инициализация UI после авторизации ---
